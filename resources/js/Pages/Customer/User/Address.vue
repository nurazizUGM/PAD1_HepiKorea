...
const addAddress = async () => {
    try {
        await axios.post('/api/address', formData.value);
...
const updateAddress = async () => {
    try {
        await axios.post(`/api/address/${editingAddress.value.id}`, formData.value);
...
const deleteAddress = async () => {
    try {
        await axios.delete(`/api/address/${editingAddress.value.id}`);
...
<template>
    <Layout title="Address">
        <div
            class="flex flex-col flex-wrap p-2 lg:p-6 border-2 bg-[#EFEFEF] border-gray-200 rounded-lg lg:rounded-3xl w-full min-h-fit">
            <div
                class="w-full h-full min-h-[82vh] lg:min-h-[640px] flex flex-col py-1 px-3 md:py-2 md:px-5 lg:p-8 bg-white rounded-3xl">
                <!-- Header -->
                <div class="flex flex-row items-center w-full mb-4">
                    <h1 class="text-[#3E6E7A] font-semibold text-[10px] md:text-sm lg:text-2xl w-fit mr-4 lg:mr-6">
                        Customer Address</h1>
                    <h2 class="text-black font-bold text-[10px] md:text-sm lg:text-xl">New Address</h2>
                    <button
                        class="rounded-full bg-[#3E6E7A] w-6 lg:w-8 hover:bg-[#37626d] active:bg-[#325862] my-auto ml-4"
                        @click="openAddModal">
                ...
                <div class="h-auto w-full mb-6 overflow-y-auto max-h-screen">
                    <div v-for="address in addresses" :key="address.id"
                        class="w-full h-full flex flex-row mt-1 lg:mt-5 mb-auto">
                        <div class="w-8/12 md:w-3/12 lg:w-2/12 flex h-full">
                            ...
                            <div class="flex flex-col ml-3 md:ml-4">
                                ...
                                <p class="text-[#898383] font-semibold text-[10px] lg:text-lg">
                                    <span class="text-orange-500 font-semibold text-[10px] lg:text-lg">(+62)</span>
                                    {{ address.phone?.replace(/^(0|62)/g, '') || '-' }}
                                </p>
                                <!-- address ini nanti muncul di mobile aja -->
                                <p
                                    class="text-black text-opacity-50 font-semibold text-[9px] lg:text-lg flex md:hidden">
            ...
            <Modal :show="showAddModal" @close="showAddModal = false">
                <div class="bg-white w-[70vw] md:w-[50vw] lg:w-[55vw] rounded-[30px] shadow md:px-8 lg:px-10 lg:py-5">
                    <div class="w-full h-full flex flex-col relative">
                        <h1 class="text-black font-bold text-[10px] md:text-lg lg:text-xl p-5 pb-2 lg:ml-6">New Address
                        </h1>
                        <button
                            class="absolute bg-black w-6 h-6 flex items-center justify-center rounded-full -top-1 -right-1 md:-right-8 lg:-top-5 lg:-right-10 "
            ...
            <Modal :show="showEditModal" @close="showEditModal = false">
                <div class="bg-white w-[70vw] md:w-[50vw] lg:w-[55vw] rounded-[30px] shadow md:px-8 lg:px-10 lg:py-5">
                    <div class="w-full h-full flex flex-col relative">
                        <h1 class="text-black font-bold text-[10px] md:text-lg lg:text-xl p-5 pb-2 lg:ml-6">Edit Address
                        </h1>
                        <button
                            class="absolute bg-black w-6 h-6 flex items-center justify-center rounded-full -top-1 -right-1 md:-right-8 lg:-top-5 lg:-right-10"
            ...
            <Modal :show="showSuccessAddedModal" @close="showSuccessAddedModal = false">
                <div
                    ...
                    <div class="flex flex-col lg:p-14">
                        <h1 class="text-black text-[10px] md:text-xl lg:text-xl font-medium mx-auto">Successfully Added!
                        </h1>
