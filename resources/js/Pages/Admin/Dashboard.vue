        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-4">
          ...
          <Link :href="route('admin.product.index')"
            ...
            <div class="mb-auto flex w-full px-4 py-3 justify-between overflow-hidden">
              <p class="text-5xl md:text-5xl lg:text-6xl font-bold">{{ dashboardData.products }}</p>
          ...
          <Link :href="route('admin.order.index')"
            class="flex flex-col h-40 md:h-40 lg:h-48 bg-gray-50 rounded-xl cursor-pointer hover:shadow-md transition-shadow">
            <div class="mb-auto flex w-full px-4 py-3 justify-between overflow-hidden">
              <p class="text-5xl md:text-5xl lg:text-6xl font-bold">{{ dashboardData.totalOrder }}</p>
          ...
          <Link :href="route('admin.customer.index')"
            class="flex flex-col h-40 md:h-40 lg:h-48 bg-gray-50 rounded-xl cursor-pointer hover:shadow-md transition-shadow">
            <div class="mb-auto flex w-full px-4 py-3 justify-between overflow-hidden">
              <p class="text-5xl md:text-5xl lg:text-6xl font-bold">{{ dashboardData.customers }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          ...
          <Link :href="route('admin.order.index')"
            ...
            <div class="w-full h-[15%] text-center mb-auto pt-3">
              <h1 class="text-black font-bold text-xl lg:text-2xl">Product Ordered</h1>
            </div>
            <div class="w-full h-[85%] relative flex items-center justify-center pb-3">
              ...
              <div class="absolute flex flex-col items-center justify-center text-center z-10">
                <h2 class="text-3xl lg:text-7xl font-bold text-black text-opacity-50">{{ dashboardData.completionRate }}%</h2>
                <h3 class="md:text-xl lg:text-2xl font-semibold text-black text-opacity-50">Complete</h3>
              </div>
            </div>
          </Link>
          ...
          <Link :href="route('admin.category.index')"
            class="flex flex-col items-center justify-center bg-gray-50 h-[298px] md:h-[298px] lg:h-[420px] rounded-xl cursor-pointer hover:shadow-md transition-shadow">
            <div class="w-full h-[15%] text-center mb-auto pt-3">
              <h1 class="text-black font-bold text-xl lg:text-2xl">Most Ordered</h1>
            </div>
            <div class="w-full h-[85%] relative flex items-center justify-center pb-3">
  ...
  export default {
    ...
    setup() {
      ...
      const dashboardData = ref({
        ...
        completedOrder: 75,
        categories: [
          { name: 'Electronics', total_order: 100 },
          { name: 'Clothing', total_order: 80 },
          { name: 'Books', total_order: 60 },
          { name: 'Home Appliances', total_order: 50 },
          { name: 'Toys', total_order: 40 },
        ],
      });
      ...
      const fetchDashboardData = async () => {
        try {
          ...
          dashboardData.value = {
            ...
            uncompletedOrder: data.uncompletedOrder,
      ...
      // Function to get cookie (for CSRF token)
        ...
        if (productOrderedChart.value) {
          new Chart(productOrderedChart.value.getContext('2d'), {
            ...
            data: {
              ...
              datasets: [{
                data: [dashboardData.value.uncompletedOrder, dashboardData.value.completedOrder],
      ...
      const initMostOrderedChart = () => {
        if (mostOrderedChart.value) {
          new Chart(mostOrderedChart.value.getContext('2d'), {
            type: 'pie',
            data: {
              labels: dashboardData.value.categories.map(category => category.name),
              datasets: [{
                label: 'percent',
                data: dashboardData.value.categories.map(category => category.total_order),
                backgroundColor: [
                  'rgb(246, 223, 170)',
                  'rgb(255, 255, 255)',
                  'rgb(49, 89, 100)',
