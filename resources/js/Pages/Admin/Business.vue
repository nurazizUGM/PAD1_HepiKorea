...
const closeEditModal = () => {
    ...
    fetchSettings(); // Refresh settings after closing modal
};

// Update setting
const updateSetting = async () => {
    try {
        axios.post(`/api/setting/${editingSetting.value.id}`, formData.value)
            .then(() => {
                closeEditModal();
            });
    } catch (error) {
...
const fetchSettings = async () => {
    try {
        const response = await axios.get('/api/setting/settings');
...
<template>
    <Layout title="Test Bisnis">
        <div
            class="p-1 md:p-4 border-2 bg-[#EFEFEF] border-gray-200 rounded-lg dark:border-gray-700 h-[88vh] overflow-auto">
            <!-- Content Container -->
            <div
                class="flex flex-col items-start justify-start bg-gray-50 h-auto dark:bg-gray-800 rounded-xl shadow-lg p-0.5 md:p-4">
                <!-- Settings Table -->
                <table class="w-full table-auto text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="setting in settings" :key="setting.id"
                            ...
                            <th scope="row"
                                class="px-1 py-2  md:px-6 md:py-4 font-medium text-gray-900 whitespace-normal dark:text-white">
                                {{ setting.name }}
                            </th>
                            <td class="py-2 md:px-6 md:py-4 whitespace-normal text-xs md:text-base">{{ setting.value }}
                            </td>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="md:px-6 md:py-3">
                                            ...
                                            <textarea v-model="formData.value" id="value" placeholder="Setting Value"
                                                rows="4"
