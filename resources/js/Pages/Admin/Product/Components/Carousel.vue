<template>
  <div class="h-full flex flex-col">
    ...
    <div
      ...
      <div v-for="carousel in carousels" :key="carousel.id"
        ...
        <div class="w-full h-2/3 bg-cover">
          <img v-if="carousel.media_type === 'image'" :src="carousel.media" class="w-full h-full object-cover" />
          <video v-else-if="carousel.media_type === 'video'" :src="carousel.media" class="w-full h-full object-cover"
            controls muted />
          <iframe v-else-if="carousel.media_type === 'youtube'" :src="carousel.media" class="w-full h-full object-cover"
            frameborder="0"
        ...
        <div class="flex mt-auto mx-4 mb-4">
          <button @click="openEditModal(carousel)" class="mr-auto">
            <img src="/img/assets/icon/icon_admin_category_edit.svg" alt="edit" class="w-5 h-5 lg:w-7 lg:h-7" />
          </button>
          <button @click="openDeleteModal(carousel.id)" class="ml-auto">
            <img src="/img/assets/icon/icon_admin_category_trash.svg" alt="delete" class="w-4 h-5 lg:w-7 lg:h-7" />
          </button>
        </div>
      </div>
    </div>
    ...
    <Modal :show="showAddModal" @close="showAddModal = false">
      <div class="bg-white w-[258px] md:w-[480px] lg:w-[30vw] h-auto rounded-[15px] md:rounded-xl lg:rounded-2xl shadow p-2 md:p-4">
        <button @click="showAddModal = false"
          class="absolute bg-black w-5 h-5 flex items-center justify-center rounded-full pb-3 -top-2 -right-2">
          <p class="m-auto text-white text-sm">X</p>
        </button>
        <form @submit.prevent="addCarousel" class="flex flex-col h-full text-center md:px-2 md:py-2 lg:py-3 lg:px-3">
          <div class="relative w-full h-56 bg-gray-200 rounded-2xl">
            <input ref="addMediaInput" type="file" :accept="addForm.media_type === 'image' ? 'image/*' : 'video/*'"
              class="hidden" @change="handleAddMedia" />
            ...
            <label @click="triggerAddMediaInput"
              ...
              <img src="/img/assets/icon/icon_admin_category_upload.svg" alt="Upload Icon" class="h-6 w-6" />
            </label>
          </div>
    ...
    <Modal :show="showDeleteModal" @close="showDeleteModal = false">
      <div class="bg-white w-[225px] md:w-[325px] lg:w-[33vw] h-auto rounded-[30px] shadow p-4">
        <div class="flex flex-col md:py-2 lg:p-10">
          ...
          <p class="text-[#B7B7B7] font-medium text-[10px] lg:text-xs mx-auto mt-2 lg:mt-6">You won’t be able to revert
            this!</p>
    ...
    <Modal :show="showSuccessModal" @close="showSuccessModal = false">
      <div class="bg-white w-[172px] md:w-[400px] lg:w-[25vw] h-auto rounded-[30px] shadow py-7 md:py-14 lg:p-4">
        <div class="flex flex-col lg:p-14">
          <h1 class="text-black text-[10px] md:text-xl lg:text-xl font-medium mx-auto">Successfully {{ successMessage }}!</h1>
...
export default {
  ...
  setup() {
    // Dummy data
    const carousels = ref([
      { id: 1, title: 'Promo 1', media: 'https://placehold.co/200', media_type: 'image', description: 'Promo image' },
      { id: 1, title: 'Promo 1', media: 'https://placehold.co/200', media_type: 'image', description: 'Promo image' },
      { id: 1, title: 'Promo 1', media: 'https://placehold.co/200', media_type: 'image', description: 'Promo image' },
      { id: 2, title: 'Video Ad', media: 'https://placehold.co/200', media_type: 'video', description: 'Video ad' },
      { id: 2, title: 'Video Ad', media: 'https://placehold.co/200', media_type: 'video', description: 'Video ad' },
      { id: 2, title: 'Video Ad', media: 'https://placehold.co/200', media_type: 'video', description: 'Video ad' },
      { id: 2, title: 'Video Ad', media: 'https://placehold.co/200', media_type: 'video', description: 'Video ad' },
      { id: 2, title: 'Video Ad', media: 'https://placehold.co/200', media_type: 'video', description: 'Video ad' },
    ...
    const fetchCarousels = async () => {
      try {
        /*
        const response = await axios.get('/api/admin/carousels');
    ...
    const updateCarousel = async () => {
      try {
        ...
        /*
        await axios.post(`/api/admin/carousels/${editForm.value.id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' },
        });
        */
    ...
    const confirmDelete = async () => {
      try {
        /*
        await axios.delete(`/api/admin/carousels/${carouselToDelete.value}`);
